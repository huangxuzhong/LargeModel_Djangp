...
class WebSocketManager():
    ...
    def __stat_listen(self):
        ...
        asyncio.get_event_loop().run_until_complete(websockets.serve(self.handle_connection, "localhost", 8460))
    ...
    async def _send_data(self):
        ...
        while True:
            try:
                ...
                # for clients in self.task_to_websocket.values():
                #     for index,client in enumerate(clients):
                #         try:
                #             await client.send("hiii{}".format(index))
    ...
    async def handle_connection(self,websocket, path):
        ...
        try:
            while True:
                ...
                # clients=self.task_to_websocket.get(task)
                # for client in clients:
                #     await client.send(f"{user}: {json.dumps(message)}")
        except websockets.ConnectionClosed:
