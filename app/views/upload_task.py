...
from app.models import UploadModelTask
...
class TaskSerializer(serializers.ModelSerializer):
    ...
    def get_model_name(sefl, obj):
        return obj.model.model_name if obj.model is not None else None
    ...
    class Meta:
        model = UploadModelTask

        fields = "__all__"


class UploadModelTaskViewSet(GenericViewSet):
    ...
    def upload_task_list(self, request):
        ...
        instances = models.UploadModelTask.objects.filter(q_objects)
    ...
    def delete_upload_task_by_id(self, request):
        my_id = request.query_params.get("id")
        if my_id is not None and my_id != "":
            if models.UploadModelTask.objects.filter(id=my_id).exists():
                instances = models.UploadModelTask.objects.get(id=my_id)
                instances.delete()
